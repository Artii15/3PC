...
class Logger extends Actor {
  ...
  private def printStates(): Unit = {
    printRow(1, f"${"Coordinator"}%30s" + workersStates.keys.toList.map(id => f"${s"Worker $id"}%30s").mkString(""))
    printRow(2, f"$coordinatorState%30s" + workersStates.values.map(state => f"$state%30s").mkString(""))
  }
